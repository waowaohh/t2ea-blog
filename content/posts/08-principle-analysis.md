...
|---------|---------|---------|
| $n=0$ | $f(x_0)$ | 低频语义/基础亮度 |
| $n=1$ | $f'(x_0)(x-x_0)$ | 一阶梯度/边缘信息 |
| $n=2$ | $\frac{f''(x_0)}{2!}(x-x_0)^2$ | 二阶曲率/纹理细节 |
...
T2EA 选择截断到二阶（$n=2$），即保留 $y_0, y_1, y_2$ 三个分量。这一选择并非任意，而是基于以下理论分析：
...
- $y_0$（零阶）包含约 **70-80%** 的图像能量
- $y_1$（一阶）包含约 **15-20%** 的能量
- $y_2$（二阶）包含约 **3-5%** 的能量
...
1. **数学一致性**：如果融合网络是恒等映射（即 $\text{fused}_i = y_i$），则重建结果等于原始输入
...
其中 $F_i, IR_i, VIS_i$ 是第 $i$ 阶泰勒分量。这个不等式成立的原因是：
...
泰勒编码器将高维图像压缩为**结构化的紧凑表示**（$y_0, y_1, y_2$），融合网络在这个紧凑空间中进行信息合成，最后逆泰勒重建恢复图像。
...
- $\|\nabla x - \max_i(g_i)\|_1$：确保高阶分量覆盖完整的高频信息
